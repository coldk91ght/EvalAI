...
class RunSubmissionTestClass(BaseTestClass):
    ...
    def test_run_submission_successfully(self, mock_map, mock_script_dict,
                                         mock_createdir, mock_lb,
                                         mock_shutil, mock_timezone,
                                         mock_open, mock_cf):
        ...
        temp_run_dir = "mocked/dir/submission_{}/run".format(self.submission.pk)
        submission_output = {
            "result": [{"split1": {"metric1": self.metric}}],
            "submission_metadata": {"test_metadata_field": "test_metadata"},
            "submission_result": ["test_result_one", "test_result_two"],
        }

        mock_map[challenge_pk] = mock.Mock()
        mock_map.get(challenge_pk).get.return_value = "test_annotation_file.txt"
        mock_script_dict[challenge_pk] = mock.Mock()
        mock_script_dict[challenge_pk].evaluate.return_value = submission_output
        ...
        self.assertEqual(self.submission.completed_at, ending_time)
        self.assertEqual(self.submission.output, submission_output)
        self.assertEqual(self.submission.submission_metadata_file, json.dumps(submission_output["submission_metadata"]))
        self.assertEqual(self.submission.submission_result_file, json.dumps(submission_output["submission_result"]))
